...
// DoRA magnitude vector (PEFT format). Shape [out_features], one scalar per output channel.
// Applied as: W_dora = magnitude / column_norm(W + scale*BA) * (W + scale*BA)
...
static bool lora_merge(WeightCtx *       wctx,
    ...
    // group lora_A, lora_B, and lora_magnitude_vector (DoRA) entries by GGUF base tensor name.
    // also collect per-tensor alpha scalars (ComfyUI baked format).
    std::map<std::string, const STEntry *> a_map, b_map, mag_map;
    ...
    for (const auto & kv : a_map) {
        ...
        // delta now holds W_merged = W_base + (alpha/rank)*scale * B @ A

        // DoRA (Weight-Decomposed LoRA): if a magnitude vector exists for this tensor,
        // rescale each output row by magnitude[i] / ||W_merged[i,:]||_2.
        //
        // Formula (PEFT merge_and_unload):
        //   norm[i]     = sqrt(sum_j W_merged[i,j]^2)
        //   W_dora[i,j] = magnitude[i] / norm[i] * W_merged[i,j]
        //
        // magnitude shape: [out_features] (= ne1), one scalar per output channel.
        auto mag_it = mag_map.find(gguf_name);
        if (mag_it != mag_map.end()) {
            const STEntry * em = mag_it->second;
            if (em->shape[0] != out_feat || em->n_dims != 1) {
                fprintf(stderr, "[LoRA] WARNING: magnitude shape mismatch for %s: [%lld] vs out_feat=%lld\n",
                        gguf_name.c_str(), (long long) em->shape[0], (long long) out_feat);
            } else {
                std::vector<float> mag_f32((size_t) out_feat);
                if (!lora_to_f32(st_data(st, *em), mag_f32.data(), out_feat, em->dtype)) {
                    fprintf(stderr, "[LoRA] WARNING: unsupported dtype %s for magnitude\n", em->dtype.c_str());
                } else {
                    lora_bf16_round(mag_f32.data(), out_feat);
                    for (int64_t i = 0; i < ne1; i++) {
                        float * row = delta.data() + i * ne0;
                        double  ss  = 0.0;
                        for (int64_t j = 0; j < ne0; j++) {
                            ss += (double) row[j] * (double) row[j];
                        }
                        float norm = (float) sqrt(ss);
                        if (norm > 0.0f) {
                            float k = mag_f32[(size_t) i] / norm;
                            for (int64_t j = 0; j < ne0; j++) {
                                row[j] *= k;
                            }
                        }
                    }
                    // match PEFT intermediate precision (same as delta bf16 round above)
                    lora_bf16_round(delta.data(), nel);
                }
            }
        }
    ...
    st_close(&st);
    fprintf(stderr, "[LoRA] Merged %d pairs (skipped %d, DoRA magnitudes %d), scale=%.2f\n", merged, skipped,
            (int) mag_map.size(), scale);
    return merged > 0;
}
